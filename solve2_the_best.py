# -*- coding: utf-8 -*-
"""
🚀 ULTIMATE FINANCIAL ANALYZER - HYBRID EDITION (MAX ACCURACY + OCR)
========================================================================
Комплексная система анализа финансовых отчётов:
✓ LLM-first подход (приоритет нейросети)
✓ 🔥 OCR для сканированных PDF (из Кода 1)
✓ 🔥 Мульти-парсер (pdfplumber → PyMuPDF → PyPDF2) (из Кода 1)
✓ 🔥 Авто-детекция типа PDF (текст/скан) (из Кода 1)
✓ Умная валидация против галлюцинаций (Тройная проверка)
✓ ТРЁХУРОВНЕВАЯ ИЕРАРХИЯ КОЭФФИЦИЕНТОВ
ТРЕБОВАНИЯ:
- Ollama: ollama serve
- Модель: ollama pull ministral-3:8b (или qwen2.5:7b)
- Библиотеки: pip install pdfplumber PyMuPDF pytesseract pdf2image Pillow
- Для OCR: установить Tesseract OCR
"""
import sys
import io
import re
...
# ==================== 🔥 БИБЛИОТЕКИ (MULTI-PARSER + OCR) ====================
# Layer 2: Мульти-парсер (из Кода 1)
try:
    import pdfplumber
    HAS_PDFPLUMBER = True
except ImportError:
...
LLM_MODEL = "qwen3.5:9b"  # Быстрая и точная модель
USE_LLM = True  # ✅ LLM ВКЛЮЧЕНА (ПРИОРИТЕТ)
REQUEST_TIMEOUT = 900  # 15 минут таймаут
REQUEST_TIMEOUT_OCR = 1800  # 30 минут для OCR
USE_OCR_FALLBACK = True  # 🔥 Включить OCR если PDF - скан
USE_MULTI_PARSER = True  # 🔥 Мульти-парсер включён

# 🔴 Подозрительно круглые числа — вероятные галлюцинации
SUSPICIOUS_ROUND_NUMBERS = {
    1000, 5000, 10000, 25000, 50000, 100000, 125000, 150000,
    200000, 250000, 300000, 500000, 1000000
}

# 📊 Ожидаемые диапазоны для телеком-отчётности (в млн руб.)
EXPECTED_RANGES = {
    'revenue': (100_000, 2_000_000),
    'net_income': (1_000, 500_000),
    'total_assets': (200_000, 5_000_000),
    'equity': (50_000, 2_000_000),
    'current_assets': (50_000, 1_000_000),
    'current_liabilities': (10_000, 500_000),
    'receivables': (10_000, 300_000),
    'cash_and_equivalents': (5_000, 200_000),
    'inventories': (5_000, 200_000),
    'operating_profit': (10_000, 500_000),
    'capex': (10_000, 500_000),
    'cost_of_goods': (50_000, 1_500_000),
}
...
class FinancialData:
    ...
    company_name: str = ""
    is_scanned_pdf: bool = False  # 🔥 Из Кода 1
    ocr_pages_count: int = 0      # 🔥 Из Кода 1
    parser_used: str = ""         # 🔥 Из Кода 1
...
# ==================== 🔥 LAYER 1: PDF TYPE DETECTOR (ИЗ КОДА 1) ====================
class PDFTypeDetector:
    """Layer 1: Авто-детекция типа PDF (текст/скан)"""
    @staticmethod
    def detect_pdf_type(pdf_path: str) -> Tuple[str, float]:
        """Определяет тип PDF. Возвращает: (тип, уверенность)"""
        try:
            if HAS_PDFPLUMBER:
                with pdfplumber.open(pdf_path) as pdf:
                    ...
                    avg_chars_per_page = text_chars / min(total_pages, 5)
...
# ==================== 🔥 LAYER 2: MULTI-PARSER (ИЗ КОДА 1) ====================
class MultiParser:
    """Layer 2: Мульти-парсер (pdfplumber → PyMuPDF → PyPDF2)"""
    def __init__(self):
        self.parsers_available = []
        if HAS_PDFPLUMBER:
            self.parsers_available.append('pdfplumber')
        if HAS_PYMUPDF:
            self.parsers_available.append('pymupdf')
        if HAS_PYPDF2:
            self.parsers_available.append('pypdf2')
        logger.info(f"📚 Доступные парсеры: {self.parsers_available}")

    def extract_text_and_tables(self, pdf_path: str) -> Tuple[str, List[Dict], str]:
        """Извлекает текст и таблицы через лучший доступный парсер"""
        for parser_name in ['pdfplumber', 'pymupdf', 'pypdf2']:
            if parser_name not in self.parsers_available:
                continue
            try:
                logger.info(f"🔍 Используем парсер: {parser_name}")
                if parser_name == 'pdfplumber':
                    text, tables = self._extract_with_pdfplumber(pdf_path)
                elif parser_name == 'pymupdf':
                    text, tables = self._extract_with_pymupdf(pdf_path)
                else:
                    text, tables = self._extract_with_pypdf2(pdf_path)
                if text.strip() or tables:
                    return text, tables, parser_name
            except Exception as e:
                logger.warning(f"⚠️ Парсер {parser_name} ошибся: {e}")
                continue
        return "", [], "none"
    ...
    def _extract_with_pdfplumber(self, pdf_path: str) -> Tuple[str, List[Dict]]:
        ...
        with pdfplumber.open(pdf_path) as pdf:
            for page_num, page in enumerate(pdf.pages, 1):
                ...
                table_bboxes = [t['bbox'] for t in tables if t['page'] == page_num]
    ...
    def _extract_with_pymupdf(self, pdf_path: str) -> Tuple[str, List[Dict]]:
        text_parts = []
        tables = []
    ...
    def _extract_with_pypdf2(self, pdf_path: str) -> Tuple[str, List[Dict]]:
        text_parts = []
        with open(pdf_path, 'rb') as f:
            reader = PyPDF2.PdfReader(f)
...
# ==================== 🔥 ULTIMATE PDF EXTRACTOR (HYBRID) ====================
class UltimatePDFExtractor:
    """Ультимативный парсер: Multi-Parser + OCR + Таблицы"""
    def __init__(self):
        self.multi_parser = MultiParser() if USE_MULTI_PARSER else None
    ...
    def extract(self, pdf_path: str) -> str:
        """Извлекает полный текст из PDF с поддержкой OCR и мульти-парсера"""
        full_text_parts = []
        
        # ═══════════════════════════════════════════════════════════
        # LAYER 1: Детекция типа PDF
        # ═══════════════════════════════════════════════════════════
        pdf_type, type_confidence = self.pdf_type_detector.detect_pdf_type(pdf_path)
        logger.info(f"📄 Тип PDF: {pdf_type} (уверенность: {type_confidence:.2f})")
        self.is_scanned = (pdf_type == "scanned")

        # ═══════════════════════════════════════════════════════════
        # LAYER 2: Мульти-парсер
        # ═══════════════════════════════════════════════════════════
        text = ""
        tables = []
        if self.multi_parser:
            text, tables, self.parser_used = self.multi_parser.extract_text_and_tables(pdf_path)
            logger.info(f"✅ Парсер: {self.parser_used}, таблиц: {len(tables)}")
        
        # ═══════════════════════════════════════════════════════════
        # LAYER 1b: OCR для сканов (если мало текста)
        # ═══════════════════════════════════════════════════════════
        if self.is_scanned and HAS_OCR and USE_OCR_FALLBACK:
            logger.info("📷 Запуск OCR для сканированного PDF...")
            ocr_text, ocr_count = self._run_ocr(pdf_path)
            if ocr_text:
                text = ocr_text if not text else text + "\n" + ocr_text
                self.ocr_pages = ocr_count
                logger.info(f"✅ OCR распознал {ocr_count} страниц")

        # ═══════════════════════════════════════════════════════════
        # Форматирование таблиц в Markdown (из Кода 2)
        # ═══════════════════════════════════════════════════════════
        current_section = "GENERAL"
        prev_table_cols = 0
        
        # Разбиваем текст по страницам для контекста
        pages_text = text.split('[СТРАНИЦА')
        for page_segment in pages_text:
            if not page_segment.strip():
                continue
            
            # Извлекаем номер страницы
            page_num_match = re.match(r'\s*(\d+)\]', page_segment)
            page_num = int(page_num_match.group(1)) if page_num_match else 0
            page_content = page_segment.split(']', 1)[1] if ']' in page_segment else page_segment
            
            # Определяем раздел
            text_lower = page_content.lower()[:1000]
            for section, keywords in SECTION_KEYWORDS.items():
                if any(kw in text_lower for kw in keywords):
                    current_section = section
                    break
            
            chunk_text = f"[РАЗДЕЛ: {current_section}] [СТРАНИЦА {page_num}]\n{page_content}\n"
            
            # Добавляем таблицы если есть
            for table in tables:
                if table['page'] == page_num:
                    max_cols = max(len(row) for row in table['cells'] if row) if table['cells'] else 0
                    is_continuation = (max_cols == prev_table_cols)
                    md_table = self._format_table(table['cells'], max_cols, is_continuation)
                    if md_table:
                        chunk_text += f"\n{md_table}\n"
    ...
    def _run_ocr(self, pdf_path: str) -> Tuple[str, int]:
        """OCR для сканированных PDF (из Кода 1)"""
        if not HAS_OCR:
            return "", 0
        try:
            ...
            for page_num, image in enumerate(images, 1):
                if page_num % 5 == 0:
                    logger.info(f"📷 OCR: страница {page_num}/{len(images)}")
...
# ==================== DATA EXTRACTOR ====================
class DataExtractor:
    """Извлечение финансовых данных из PDF (LLM-first + Multi-Parser + OCR)"""
    def __init__(self):
    ...
    def extract_from_pdf(self, pdf_path: str) -> FinancialData:
        """🆕 Извлечение данных из PDF с использованием UltimatePDFExtractor (OCR + Multi-Parser)"""
        data = FinancialData()
        try:
            pdf_extractor = UltimatePDFExtractor()
            text = pdf_extractor.extract(pdf_path)
            
            # Сохраняем метаданные из экстрактора
            data.is_scanned_pdf = pdf_extractor.is_scanned
            data.ocr_pages_count = pdf_extractor.ocr_pages
            data.parser_used = pdf_extractor.parser_used
...
class ReportGenerator:
    ...
    def generate_text_report(result: AnalysisResult) -> str:
        report = []
        report.append("=" * 100)
        report.append("ФИНАНСОВЫЙ АНАЛИЗ - HYBRID EDITION (3 УРОВНЯ + OCR)".center(100))
        report.append("=" * 100)
        ...
        if result.financial_data.is_scanned_pdf:
            report.append(f"📷 OCR: ИСПОЛЬЗОВАН (сканированный PDF, {result.financial_data.ocr_pages_count} стр.)")
        report.append(f"📚 Парсер: {result.financial_data.parser_used}")
    ...
    def generate_json_report(result: AnalysisResult) -> str:
        data_dict = {
            'metadata': {
                ...
                'is_scanned': result.financial_data.is_scanned_pdf,
                'ocr_pages': result.financial_data.ocr_pages_count,
                'parser_used': result.financial_data.parser_used,
            },
...
class FinancialAnalysisEngine:
    ...
    def analyze(self, pdf_path: str, company_name: str = "") -> AnalysisResult:
        ...
        print(f"\n🚀 ULTIMATE FINANCIAL ANALYZER (HYBRID: 3-УРОВНЯ + OCR + MULTI-PARSER)")
        print(f"📄 Файл: {pdf_file.name}")
        print(f"📅 Дата: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"🤖 LLM Модель: {LLM_MODEL} (Ollama)\n")
        # ===== УРОВЕНЬ 0: Извлечение исходных данных =====
        print("=" * 80)
        print("📊 УРОВЕНЬ 0: Извлечение исходных данных (LLM + OCR + Multi-Parser)")
        print("=" * 80)
        extractor = DataExtractor()
        financial_data = extractor.extract_from_pdf(str(pdf_path))
        print(f"✅ Обработано страниц: {extractor.total_pages}")
        print(f"✅ Скан: {financial_data.is_scanned_pdf}, OCR страниц: {financial_data.ocr_pages_count}")
        print(f"✅ Парсер: {financial_data.parser_used}\n")
        # ===== УРОВЕНЬ 1: Расчёт первичных показателей =====
        print("=" * 80)
        ...
        print("=" * 80 + "\n")
        # Для recovery используем простой экстракт если pdfplumber доступен
        full_text = ""
        if HAS_PDFPLUMBER:
            try:
                with pdfplumber.open(pdf_path) as pdf:
...
def main():
    """Главная функция"""
    print("\n" + "="*80)
    print("ПРОВЕРКА КОНФИГУРАЦИИ LLM + OCR + MULTI-PARSER")
    print("="*80)
