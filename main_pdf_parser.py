...
🆕 УЛУЧШЕНИЯ ДЛЯ МАШИННОГО СЧИТЫВАНИЯ:
1️⃣ Структурированный JSON вывод с метаданными
2️⃣ Таблицы в формате Markdown (лучше для парсинга)
3️⃣ Умная обработка колонок (учет X-координат)
    ...
    def _extract_all_pages_surya(self, pdf_path: str, total_pages: int) -> Dict[int, Tuple[str, float, List[Dict]]]:
        ...
        try:
            ...
            for page_idx, page_pred in enumerate(pages):
                ...
                # 🆕 УМНАЯ ГРУППИРОВКА С УЧЕТОМ КОЛОНОК
                items = []
                y_tol = 6.0
...
if __name__ == "__main__":
    ...
    if result['success']:
        ...
        if 'structured' in result:
            _safe_print(f"  🆕 Structured JSON: {result['output_file'].replace('.txt', '.json')}")
        _safe_print("="*70)
    else:
