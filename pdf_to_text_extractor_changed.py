# -*- coding: utf-8 -*-
"""
📄 PDF_TO_TEXT_EXTRACTOR — Surya OCR как ОСНОВНОЙ движок
=============================================================================

🎯 ОСНОВНОЙ ДВИЖОК: Surya OCR (приоритет №1)
     Использует состояние искусства OCR для максимально точного извлечения текста
     🔥 Запускается в пакетном режиме для всех страниц сразу (оптимизация скорости)

FALLBACK парсеры (если Surya не справился):
  ✅ pdfplumber — извлечение таблиц и структурированного контента
  ✅ PyMuPDF — быстрое извлечение простого текста

ИЗ pdf_converter.py:
  ✅ NumberNormalizer — нормализация финансовых чисел
  ✅ PageRegistry — отслеживание статуса каждой страницы

ИСПОЛЬЗОВАНИЕ:
  python pdf_to_text_extractor.py input/file.pdf [max_pages]
  python pdf_to_text_extractor.py input/file.pdf 10  # первые 10 страниц

ТРЕБОВАНИЯ:
  pip install pdfplumber pymupdf 'surya-ocr>=0.17.1' pdf2image pillow
"""
...
try:
    # Проверяем наличие CLI surya_ocr в venv
    venv_surya = Path(__file__).parent / 'venv' / ('Scripts' if os.name == 'nt' else 'bin') / ('surya_ocr.exe' if os.name == 'nt' else 'surya_ocr')
    if venv_surya.exists():
        SURYA_CMD = str(venv_surya)
        HAS_SURYA = True
        logger.info(f"✅ Surya найдена в venv: {SURYA_CMD}")
    else:
...
class PDFToTextExtractor:
    ...
    def extract(self, pdf_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        try:
            ...
            full_text = self._extract_surya_first(str(pdf_path), total_pages)
            ...
            logger.info(f"✅ Готово за {elapsed:.1f}с | Символов: {len(full_text):,}")
            return result
    ...
    def _extract_surya_first(self, pdf_path: str, total_pages: int) -> str:
        """
        🔥 ИЗВЛЕЧЕНИЕ С SURYA-FIRST ЛОГИКОЙ
        ...
        """
        all_text = []
        surya_cache: Dict[int, Tuple[str, float]] = {}
        ...
        for page_num in range(1, total_pages + 1):
            ...
            if page_num in surya_cache:
                text, confidence = surya_cache[page_num]
    ...
    def _extract_all_pages_surya(self, pdf_path: str, total_pages: int) -> Dict[int, Tuple[str, float]]:
        """
        🔥 ПАКЕТНАЯ ОБРАБОТКА: Surya для ВСЕХ страниц за один запуск
        
        Возвращает: {page_num: (text, confidence)}
        """
        results = {}
        ...
        try:
            ...
            result = subprocess.run(
                cmd,
                capture_output=False,  # Показываем прогресс в консоли
                timeout=1800,  # 30 минут максимум
                env={
                    **os.environ,
                    'CUDA_VISIBLE_DEVICES': '0',
                    'HF_HOME': str(Path(tempfile.gettempdir()) / 'hf_cache')
                }
            )
            
            if result.returncode != 0:
                logger.warning(f"⚠️ Surya batch failed with code {result.returncode}")
                return results
            ...
            for page_idx, page_pred in enumerate(pages):
                ...
                # Группировка text_lines по Y-координате
                items = []
                y_tol = 6.0
                ...
                results[page_num] = (page_text, confidence)
    ...
    def _extract_page_surya_single(self, pdf_path: str, page_num: int) -> Tuple[str, float]:
        ...
        try:
            ...
            result = subprocess.run(
                ...
                text=True,
                timeout=120,
                env={**os.environ, 'CUDA_VISIBLE_DEVICES': '0', 'HF_HOME': str(Path(tempfile.gettempdir()) / 'hf_cache')}
            )
    ...
    def _clean_html_tags(self, text: str) -> str:
        ...
        Удаляет остатки HTML после Surya/pdfplumber:
        - <br>, <br/>, <br />
        - <p>, </p>, <div>, </div>, и т.д.
        - &nbsp;, &lt;, &gt;, и прочие HTML-сущности
        
        Пример:
            "637 469<br>180 175" → "637 469 180 175"
        """
    ...
    def _identify_table_regions(self, lines: List[str]) -> List[Tuple[int, int]]:
        ...
        - 2+ строки подряд с ТОЛЬКО цифрами, пробелами, разделителями (|, -, =)
        - НЕТ букв/текста (кроме названия)
        - Выглядит как несколько чисел в строке
        """
        regions = []
        i = 0
    ...
    def _recover_table_structure(self, table_lines: List[str]) -> List[str]:
        ...
        Логика:
        1. Парсим каждую строку на числовые "колонки"
        2. Выравниваем колонки по вертикали
        3. Восстанавливаем формат
        """
        ...
        for line in table_lines:
            ...
            # Находим всех чисел (включая те, что разделены пробелами)
    ...
    def _extract_numbers_from_line(self, line: str) -> List[str]:
        ...
        Логика:
        - Числа разделены 2+ пробелами (то это разные колонки)
        - Числа разделены 1 пробелом (то это части одного числа)
        
        Пример 1: "637 469 180 175"
            ✗ Неправильно: ["637", "469", "180", "175"] (разные строки)
            ✓ Правильно: ["637 469", "180 175"] (две колонки)
        
        Пример 2: "127 658  110 035"  (двойной пробел = разделитель)
            ✓ Правильно: ["127 658", "110 035"]
        """
    ...
    def _format_financial_number(self, num_str: str) -> str:
        """
        💰 ФОРМАТИРОВАНИЕ ФИНАНСОВОГО ЧИСЛА
    ...
    def _format_tables(self, tables: List[List[List[str]]]) -> str:
        """Форматирование таблиц из pdfplumber"""
        if not tables:
            return ""
        ...
        for table_idx, table in enumerate(tables):
            ...
            result.append(f"\n{'='*80}")
            result.append(f"TABLE {table_idx + 1}")
            result.append('='*80)
            ...
            col_widths = []
            for col_idx in range(max(len(row) for row in formatted_rows)):
                max_width = max(
            ...
            for row_idx, row in enumerate(formatted_rows):
                ...
                for col_idx, cell in enumerate(row):
                    ...
                    is_number = cell and any(c.isdigit() for c in cell) and not any(c.isalpha() for c in cell if c not in '()-, ')
                ...
                if row_idx == 0:
                    ...
                    for width in col_widths:
                        sep += '-' * (width + 2) + '+'
                    result.insert(len(result) - 1, sep)
    ...
    def save_to_file(self, result: Dict[str, Any], output_path: Optional[Path] = None) -> Path:
        ...
        json_path.write_text(json.dumps(result['stats'], indent=2, ensure_ascii=False), encoding='utf-8')
...
def extract_pdf(pdf_path: str, 
    ...
    Returns:
        Dict: {'success': bool, 'text': str, 'stats': dict, 'error': str}
    """
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        _safe_print("Usage: python pdf_to_text_extractor.py <pdf_file> [max_pages]")
        _safe_print("Examples:")
        _safe_print("  python pdf_to_text_extractor.py input/report.pdf")
        _safe_print("  python pdf_to_text_extractor.py input/report.pdf 10")
        _safe_print("  python pdf_to_text_extractor.py input/report.pdf --debug")
        sys.exit(1)
    ...
    if max_pages:
        _safe_print(f"📄 Processing first {max_pages} pages...")
    ...
    result = extract_pdf(
        ...
        save_output=True, 
        debug=debug,
        surya_first=True,        # 🔥 Surya как основной
        use_surya_batch=True,    # 🔥 Пакетная обработка
        min_confidence=0.7       # Порог для fallback
    )
    
    if result['success']:
        _safe_print("\n" + "="*70)
        _safe_print("✅ SUCCESS")
        _safe_print(f"  File: {result['filename']}")
        _safe_print(f"  Pages: {result['total_pages']}")
        _safe_print(f"  Characters: {result['char_count']:,}")
        _safe_print(f"  Time: {result['extraction_time_sec']}s")
        _safe_print(f"  Coverage: {result['stats']['coverage_percent']}%")
        _safe_print(f"  Methods: {', '.join(result['stats']['methods'])}")
        if 'output_file' in result:
            _safe_print(f"  Output: {result['output_file']}")
        _safe_print("="*70)
    else:
